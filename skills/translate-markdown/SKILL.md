...
license: MIT
compatibility: Requires Python 3.10+. No PDF tooling involved. Translation needs an OpenAI-compatible endpoint and API key (PDF2MD_* or OPENAI_* environment variables, a .pdf2obsidian.json config file, or pi's own provider config). Tested on Windows and Linux.
---
...
.pdf2obsidian/my-note/         blocks.jsonl, meta.json, translate.sqlite
```
...
comes from flags, environment variables (`PDF2MD_*` / `OPENAI_*`) or a
`.pdf2obsidian.json` config file. Reading pi's config is a last-resort fallback.
...
scripts/pdf2obsidian.py   renderer (blocks.jsonl -> note.md)
